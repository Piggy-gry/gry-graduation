...
NIMA_checkpoint_path = '/home/lab325/gry/Neural-IMage-Assessment/train/save_checkpoints/his_checkpoint/epoch-82.pth'


def load_NIMA_state_dict(model, checkpoint_path):
    ...
    #     count += 1
    print('Successfully loaded model epoch-82.pth')
    return model
...
def main(config):
    ...
    model.load_state_dict(torch.load(NIMA_checkpoint_path), strict=False)
...
def test():
    model = VGG16BinaryNet()
    model = load_NIMA_state_dict(model, NIMA_checkpoint_path)

    param_num = 0
    for param in model.parameters():
        if param.requires_grad:
            param_num += param.numel()
    print('initial Trainable params: %.2f million' % (param_num / 1e6))
    ...
    for param in model.parameters():
        if param.requires_grad:
            param_num += param.numel()
    print('breeze Trainable params: %.2f million' % (param_num / 1e6))
...
if __name__ == '__main__':
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='./checkpoint/test')
